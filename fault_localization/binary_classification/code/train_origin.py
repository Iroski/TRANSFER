...
from model_new import BinaryClassifier
import numpy

### get embedding for a sample of code
def get_embedding(tokens,vectors,method='default'): # default , identifier , all
    result=[]
    for i in range(len(tokens)):
        token=tokens[i]
        if type(token)==int:
            result.append(vectors[token])
        else:
            if method=='default':
                result.append(vectors[token[2]])
            elif method=='identifier':
                tmp_list=[]
                for j in range(len(token[0])):
                    identifier=token[0][j]
                    tmp_list.append(vectors[identifier])
                final_token=numpy.mean(tmp_list,axis=0)*0.5+vectors[token[2]]*0.5
                result.append(final_token)
            else:
                pass #todo
    return result

def get_batch(x, y, idx, bs,vectors):
	x_batch = x[idx: idx+bs]
	x_embedded=[get_embedding(sample,vectors) for sample in x_batch]
	y_batch = y[idx: idx+bs]
	return torch.FloatTensor(x_embedded), torch.LongTensor(y_batch)
...
if __name__ == "__main__":
	...
	model = BinaryClassifier(EMBEDDING_DIM, HIDDEN_DIM, MAX_TOKENS, LABELS)
	...
	for epoch in range(EPOCHS):
		...
		i = 0
		while i < len(val_x):
			batch = get_batch(val_x, val_y, i, BATCH_SIZE,pretrain_vectors)
		...
		if val_acc_[-1] > best_val_acc:
			model_save_dir = "./model_save_default/{}/".format(fix_pattern)
	...
	model.load_state_dict(torch.load("./model_save_new/{}/model_params.pkl".format(fix_pattern)))
	model.eval()

	i = 0
	while i < len(test_x):
		batch = get_batch(test_x, test_y, i, BATCH_SIZE,pretrain_vectors)
