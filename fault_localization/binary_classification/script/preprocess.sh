...
for i in MutateDataType  MutateLiteralExpr  MutateMethodInvExpr  MutateOperators  MutateReturnStmt  MutateVariable  RemoveBuggyStmt ;

do
    echo "--------------------------------"
    echo $i
    python data_process_new.py $i
    if [ $? -ne 0 ]; then
        echo "[ERROR] "
        exit 1
    fi
